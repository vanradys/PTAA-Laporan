...
-- Pada versi lama, "deadline/Tanggal Delivery" dipakai sebagai target.
-- target_penyelesaian dipertahankan sebagai sumber aktual lama bila pernah diisi.
UPDATE projects_po
SET
  target_pengiriman = COALESCE(NULLIF(target_pengiriman, ''), NULLIF(deadline, '')),
  aktual_pengiriman = COALESCE(
    NULLIF(aktual_pengiriman, ''),
    NULLIF(target_penyelesaian::text, '')
  )
