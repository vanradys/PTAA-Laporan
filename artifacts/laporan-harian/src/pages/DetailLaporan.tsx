...
export default function DetailLaporan() {
  ...
  const handleTaskCorrection = async (task: Task) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getGetReportQueryKey(reportId) });
      queryClient.invalidateQueries({ queryKey: getListReportsQueryKey() });
      toast({ title: "Perbaikan tersimpan", description: "Status tugas berubah menjadi Sudah Diperbaiki." });
    } catch (error) {
                          ...
                          {task.reviewStatus && (
                            <div className="mb-2">
                              <span className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${
                                ...
                                {task.reviewStatus === "revisi" ? "Revisi" : task.reviewStatus === "sudah_diperbaiki" ? "Sudah Diperbaiki" : "Direview"}
                              </span>
                              {task.reviewComment && <p className="mt-1 max-w-xs text-xs text-slate-600">{task.reviewComment}</p>}
